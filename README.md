# Sum-body, Stop me!
## FEATURES:
#### 1. sum will be between 2 and 100
#### 2. Dark themed
#### 3. best for math lovers
#### 4. You will have 10 seconds to give  the answer
#### 5. Giving the answer in less than 7 seconds will give you 2 points
#### 6. Rules are given on the landing apge
#### 7. Five lives will be given
## LANGUAGES:
- Html
- css
- javascript
## HOW TO PLAY?
#### Open INDEX.HTML from the folder
#### press start to start and done to submit your answer.
## Live link
https://nabaafatima.github.io/math-quiz/
<img width="1438" height="743" alt="image" src="https://github.com/user-attachments/assets/30b8c6bf-e9c2-4814-9162-8c4ec61367da" />


